...
let AppComponent = class AppComponent {
    constructor(http, platform, splashScreen, statusBar, router, toastController, location, androidPermissions, 
    ...
    initializeApp() {
        ...
        this.platform.ready().then(() => {
            ...
            let permissions = [this.androidPermissions.PERMISSION.CAMERA, this.androidPermissions.PERMISSION.GET_ACCOUNTS, this.androidPermissions.PERMISSION.READ_EXTERNAL_STORAGE,
...
let GroupbyPipe = class GroupbyPipe {
    ...
    // }
    transform(base) {
        let seconds = Math.floor(base * this.milliseconds / 1000);
        ...
        let totalhours = getMinutes + ':' + getSeconds;
        return totalhours;
    }
};
...
const SQL_SELECT_ALL_CREDENTIALS = `SELECT * FROM credentials`;
const SQL_SELECT_ALL_SETTINGS = `SELECT * FROM settings`;
const SQL_SELECT_ALL_EVENTS = `SELECT * FROM events`;
const SQL_SELECT_ALL_ENUMS = `SELECT * FROM enum_masters`;
const SQL_SELECT_ALL_HEALTH_DETAILS = `SELECT * FROM health_details`;
const SQL_SELECT_ALL_USERS = `SELECT * FROM users`;
const SQL_SELECT_ALL_EMERGENCY_DATA = `SELECT * FROM emergency_details`;
const SQL_TABLES = ["emergency_details", "enum_masters", "events", "health_details", "users", "user_associations"];
;
;
;
;
...
let syncProvider = class syncProvider {
    ...
    updateImageDeletion() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            let sqlEventQuery = `SELECT * FROM events WHERE ((event_type='alert_medication' OR event_type='report' OR event_type='prescription') AND delete1='false')`;
            return this.getDatabase().then((database) => {
                return database.executeSql(sqlEventQuery, []).then((data) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                    for (let i = 0; i < data.rows.length; i++) {
                        ...
                        if (index.length > 0) {
                            ...
                            yield this.deleteEventImages(params).subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        ...
                        if (length > 0) {
                            ...
                            for (let i = 0; i < data.rows.length; i++) {
                                ...
                                if (data.rows.item(i).id == null) {
                                    ...
                                    yield this.createSingleEventData(rowData);
                                }
                                else {
                                    if (data.rows.item(i).delete1 == 'true') {
                                        // console.log('delete',data.rows.item(i).delete1)
                                        yield this.deleteSingleEventData(rowData);
                                    }
                                    else {
                                        let id = data.rows.item(i).id;
                                        let findindex = this.allEvents.findIndex(res => { return res.id == id; });
                                        ...
                                        if (findindex != -1 && this.allEvents[findindex]['updated_at'] > data.rows.item(i).updated_at) {
                                            ...
                                            yield this.updateSingleEventData(rowData);
                                        }
                                    }
                                }
    ...
    getUniqueEventDataPush() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            let getUserIds = yield this.getUserIdForEvents();
            this.getDatabase().then((database) => {
                this.getAllEventsList(getUserIds).subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                    let array1 = responseList[0]['events'];
                    let array2 = yield database.executeSql(`SELECT * FROM events`, []).then((data) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        let events = [];
                        for (let i = 0; i < data.rows.length; i++) {
                            let event_json = null;
                            let eventAssetsJson = null;
                    ...
                    let unique = yield this.findUniqueOnOnline(array1, array2);
                    for (let j = 0; j < unique.length; j++) {
                        ...
                        if ((unique.length - 1) == j) {
                            let unique1 = yield this.findUniqueOffline(array2, array1);
                            this.deleteDataIfnotExistsonOnline(unique1);
                        }
                    }
                    if (unique.length == 0) {
                        let unique1 = yield this.findUniqueOffline(array2, array1);
                        yield this.deleteDataIfnotExistsonOnline(unique1);
                    }
                }));
            });
        });
    }
    findUniqueOffline(first, second) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            let uniq = [];
    ...
    awaitAllUsersTableData() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            //if(this.isNetworkOnline == true){  
            this.getDatabase().then((database) => {
                this.requestDataFromMultipleSources().subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        ...
                        if (length > 0) {
                            ...
                            for (let i = 0; i < data.rows.length; i++) {
                                ...
                                if (data.rows.item(i).id == null && profile_id == data.rows.item(i).user_id) {
                                    yield this.updateSingleEmergencyDetail(rowData);
                                }
                                else {
                                    if (data.rows.item(i).delete1 == 'true' && profile_id == data.rows.item(i).user_id) {
                                        yield this.deleteSingleEmergencyDetail(rowData);
                                    }
                                    else {
                    ...
                    yield database.executeSql(`SELECT * FROM health_details`, []).then((data) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        let length = data.rows.length;
                        if (length > 0) {
                            let healthData = [];
                            for (let i = 0; i < data.rows.length; i++) {
                                ...
                                let index = this.responseData2.findIndex(res => { return res.id == id; });
                                console.log(index);
                                if (index != -1) {
                                    if (data.rows.item(i).id == null && data.rows.item(i).name == "health" && profile_id == data.rows.item(i).user_id || data.rows.item(i).updated_at > this.responseData2[index]['updated_at'] && data.rows.item(i).name == "health" && profile_id == data.rows.item(i).user_id) {
                                        ...
                                        yield this.createSingleHealthDetail(rowData);
                                    }
                                        ...
                                        yield this.createSingleHealthDetail(this.responseData2[index]);
                                    }
                                }
                            }
                    ...
                    yield database.executeSql(`SELECT * FROM health_details WHERE name='policy'`, []).then((data) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        ...
                        if (length == 0) {
                            ...
                            let userData = [];
                            yield database.executeSql(sqlUserQuery, []).then((data2) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        ...
                        if (length > 0) {
                            for (let i = 0; i < data.rows.length; i++) {
                                ...
                                if (data.rows.item(i).delete1 == 'true' && data.rows.item(i).role_id == 2) {
                                    ...
                                    let id = data.rows.item(i).id;
                                    let findindex = this.responseData3.findIndex((res) => { return res.id == id; });
                                    ...
                                    console.log(this.responseData3[findindex]['updated_at'] > data.rows.item(i).updated_at);
                                    console.log(this.responseData3[findindex]['updated_at'], data.rows.item(i).updated_at);
                                    if (findindex != -1 && this.responseData3[findindex]['updated_at'] > data.rows.item(i).updated_at) {
                                        ...
                                        let putId = data.rows.item(i).id;
                                        console.log(putId);
                                        yield this.updateUsersBasicsData(putId, createEventData).subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                                        }));
                                        //let updateUserssql = `UPDATE users SET name = ?, email = ?, mobile_no = ? WHERE id = ?`;      
                                        if (profile_picture['url'] != null && profile_picture['toUpdate'] == true && profile_picture['localURL'] != null) {
                                            ...
                                            let uploadStatus = yield this.uploadImage(localPath, params, requestUrl, "user", "user_picture");
                                            ...
                                            if (uploadStatus["status"] == "true") {
                                                ...
                                                let user_option = uploadStatus['user_option'];
    ...
    getUniqueUsersDataPush() {
        this.getDatabase().then((database) => {
            this.requestDataFromMultipleSources().subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                ...
                let array1 = yield database.executeSql(`SELECT * FROM emergency_details`, []).then((data) => {
                    let emergencyContacts = [];
                    for (let i = 0; i < data.rows.length; i++) {
                        emergencyContacts.push({
                            id: data.rows.item(i).id,
                ...
                let array2 = yield database.executeSql(`SELECT * FROM health_details`, []).then((data1) => {
                    let healthData = [];
                ...
                let array3 = yield database.executeSql(`SELECT * FROM users`, []).then((data2) => {
                    let userData = [];
                    for (let i = 0; i < data2.rows.length; i++) {
                ...
                let unique1 = yield this.findUniqueOnOnline(this.emegencyConatcts, array1);
                let unique2 = yield this.findUniqueOnOnline(this.healtDetails, array2);
                let unique3 = yield this.findUniqueOnOnline(this.usersList, array3);
                console.log(unique1);
                console.log(unique2);
                console.log(unique3);
                if (unique1.length > 0) {
                    ...
                    let uniqueData1 = yield this.findUniqueOffline(array1, this.emegencyConatcts);
                ...
                if (unique2.length > 0) {
                    ...
                    let uniqueData2 = yield this.findUniqueOffline(array2, this.healtDetails);
                ...
                if (unique3.length > 0) {
                    ...
                    let uniqueData3 = yield this.findUniqueOffline(array3, this.usersList);
    ...
    findUniqueOnOnline(first, second) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            let uniq = [];
    ...
    getEmergencyContacts(response, unique, array2, array1) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.getDatabase().then((database) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (unique != "unique") {
    ...
    getHealthDetails(response, unique, array2, array1) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.getDatabase().then((database) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (unique != "unique") {
    ...
    getUsersData(response, unique, array2, array1) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.getDatabase().then((database) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                if (unique != "unique") {
                ...
                for (let i = 0; i < allUsers.length; i++) {
                    ...
                    let user_option_json = JSON.stringify(allUsers[i]['user_option']);
                    let data3 = [
                        allUsers[i]["id"],
                        allUsers[i]["name"],
                        allUsers[i]["email"],
    ...
    createSingleHealthDetail(rowData) {
        ...
        return this.updateHealthData(data).subscribe((responseList) => {
            ...
            let createEventData = [response["id"], JSON.stringify(response["attribute_name_value"]), response["created_at"], response["updated_at"], rowData["health_id"]];
            return this.commonUpdateAndDeleteEvent(sql, createEventData);
        });
    }
        ...
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (data['event_type'] != 'vital') {
                return this.insertEventData(data).subscribe((responseList) => {
                    ...
                    return this.commonUpdateAndDeleteEvent(sql, createEventData).then((res) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                        let parse_event_options = JSON.parse(rowData["event_options"]);
                        if (res["event_id"] != undefined && parse_event_options["localImagePath"] != undefined && parse_event_options["localImagePath"] != null) {
                            //let localImagePath = parse_event_options["localImagePath"];
                            for (let i = 0; i < parse_event_options["localImagePath"].length; i++) {
                                if (parse_event_options["localImagePath"][i]["globalURI"] == null) {
                                    let params = { "id": response["id"] };
                                    let requestUrl = `events/update_image`;
                                    let uploadStatus = yield this.uploadImage(parse_event_options["localImagePath"][i], params, requestUrl, "event", "event_assets");
                                    console.log(uploadStatus);
                                    if (uploadStatus["status"] == "true") {
                                        ...
                                        let createEventData = [sql, [JSON.stringify(parse_event_options), rowData["event_id"]]];
                                        this.commonUpdateAndDeleteEvent(sql, createEventData);
                                    }
                                }
                            }
                        ...
                        if (res["event_id"] != undefined && rowData["event_options"]["localImagePath"] != undefined && rowData["event_options"]["localImagePath"] != null) {
                            ...
                            for (let i = 0; i < localImagePath.length; i++) {
                                if (localImagePath[i]["globalURI"] != null) {
                                    let params = { "id": response["id"] };
                                    let requestUrl = `events/update_image`;
                                    let uploadStatus = yield this.uploadImage(localImagePath[i], params, requestUrl, "event", "event_assets");
                                    console.log(uploadStatus);
                                    if (uploadStatus["status"] == "true") {
                                        let updateEventOptions = JSON.parse(rowData["event_options"]);
        ...
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            if (data['event_type'] != 'vital') {
                return this.updateEventData(rowData["id"], data).subscribe((responseList) => {
                    let response = responseList[0]['event'];
                    let sql = `UPDATE events SET event_assets = ?, created_at = ?, updated_at = ? WHERE event_id = ?`;
                    let createEventData = [response["event_assets"], response["created_at"], response["updated_at"], rowData["event_id"]];
                    return this.commonUpdateAndDeleteEvent(sql, createEventData);
                });
            }
            else {
    ...
    updateUsersData(rowData, healthData) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            return this.updateUserAndHealthData(data).subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
                let response = responseList[0];
                let policies = response['policies'];
                let userData = response['user'];
                let sql = `UPDATE health_details SET id = ?, attribute_name_value = ?, created_at = ?, updated_at = ? WHERE health_id = ?`;
                let createEventData = [policies["id"], JSON.stringify(policies["attribute_name_value"]), policies["created_at"], policies["updated_at"], healthData["health_id"]];
                console.log(createEventData);
                yield this.commonUpdateAndDeleteEvent(sql, createEventData);
                let sql1 = `UPDATE users SET age = ?, blood_group = ?, updated_at = ? WHERE id = ?`;
                let createEventData1 = [userData["age"], userData["blood_group"], userData["updated_at"], userData["id"]];
                yield this.commonUpdateAndDeleteEvent(sql1, createEventData1);
            }));
        });
    }
    getUserPolicy() {
    ...
    deletePatientData(rowData) {
        return this.deletePatientFromCaregiver(rowData["id"]).subscribe((responseList) => tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            let sql = `DELETE FROM users WHERE id = ?`;
            let createEventData = [rowData["id"]];
            yield this.commonUpdateAndDeleteEvent(sql, createEventData);
    ...
    updateUsersBasicsData(id, data) {
        let response1 = this.http.put(`users/` + id, data);
        return Object(rxjs__WEBPACK_IMPORTED_MODULE_7__["forkJoin"])([response1]);
    }
    uploadImage(localfile, params, requestMethods, user, key) {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            ...
            console.log(options);
            let getUrl;
            return fileTransfer.upload(localfile["cdvFilePath"], _environments_environment__WEBPACK_IMPORTED_MODULE_9__["environment"].apiUrl + requestMethods, options).then(res => {
    ...
    getUserIdFromCareGiver() {
        return tslib__WEBPACK_IMPORTED_MODULE_0__["__awaiter"](this, void 0, void 0, function* () {
            this.getPatientIds = [];
            return this.getPatientsList().then((responseList) => {
                this.patientList = responseList;
                for (let i = 0; i < this.patientList['patient'].length; i++) {
                    this.getPatientIds.push(this.patientList['patient'][i]['id']);
                }
